...
import {
  ...
  findPlantBundle, hashBytes, hashJson, stableJson,
...
function buildManifest({ mainsceneBundleId, plantBundleId, officialItems, officialPlants, overrides, items, plants, assets }) {
...
function validateBundle(bundleDir, { legacyItems, overrides } = {}) {
  ...
  return { manifest, items, plants };
}
function createStage({ datasets, plantBundle, plantConfig, assets, overrides, legacyItems, tempDir }) {
  const { items, plants } = buildCatalogs({ officialItems: datasets.items, officialPlants: datasets.plants, legacyItems, overrides, assets });
  const manifest = buildManifest({ mainsceneBundleId: datasets.bundleId, plantBundleId: plantBundle.bundleId, officialItems: datasets.items, officialPlants: datasets.plants, overrides, items, plants, assets });
  ...
  return { stage, manifest, items, plants };
}
...
function readPreviousCatalogs() {
  try {
    ...
    return { items: readJson(path.join(BUNDLE_DIR, manifest.catalogs.items.file)), plants: readJson(path.join(BUNDLE_DIR, manifest.catalogs.plants.file)) };
  } catch { return { items: [], plants: [] }; }
}
...
function report(result, legacyItems, overrides, previous) {
  ...
  return {
    ...
    counts: { items: result.items.length, plants: result.plants.length, logicalAssets: result.manifest.assets.count, uniqueAssets: result.manifest.assets.uniqueCount },
    changes: { items: catalogDiff(previous.items, result.items), plants: catalogDiff(previous.plants, result.plants) },
    quality: {
...
function printDiff(stage) {
  ...
  console.log(`物品 ${after.catalogs.items.count}，植物 ${after.catalogs.plants.count}，图片 ${after.assets.count}/${after.assets.uniqueCount}（逻辑/唯一）`);
...
if (command === 'scan') {
  ...
  try {
    ...
    if (dryRun) console.log('dry-run：已完成完整生成和校验，未修改仓库');
      ...
      fs.mkdirSync(path.dirname(REPORT_PATH), { recursive: true }); fs.writeFileSync(REPORT_PATH, stableJson(report(result, legacyItems, overrides, previous)));
  ...
  console.log(`资源包检查通过：物品 ${result.items.length}，植物 ${result.plants.length}，图片 ${result.manifest.assets.count}/${result.manifest.assets.uniqueCount}`);
} else {
