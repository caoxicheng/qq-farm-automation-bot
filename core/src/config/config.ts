...
function updateRuntimeConfig(newConfig: unknown): void {
    const source = asRecord(newConfig);
    if (source.serverUrl && typeof source.serverUrl === 'string') {
        CONFIG.serverUrl = source.serverUrl;
    }
    if (source.clientVersion && typeof source.clientVersion === 'string') {
        CONFIG.clientVersion = source.clientVersion;
    }
    if (source.platform && typeof source.platform === 'string') {
        CONFIG.platform = source.platform;
    }
    if (source.os && typeof source.os === 'string') {
        CONFIG.os = source.os;
    }
}
