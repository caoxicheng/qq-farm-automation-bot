...
const { getNativeWxLoginCode } = require("./native-protocol") as {
  getNativeWxLoginCode: (loginBuffer: string, appId: string) => Promise<string>;
};

type DataRecord = Record<string, unknown>;
type CookieJar = Map<string, string>;
export type WxPollStatus = "waiting" | "scanned" | "cancelled" | "expired" | "authorized";
...
export interface RotatedCredentialError extends Error {
  refreshtoken?: string;
  accesstoken?: string;
}
...
function requiredCookie(cookies: CookieJar, name: string): string {
  const value = cookies.get(name);
  if (!value) throw new Error(`WeChat OAuth callback did not provide ${name}`);
  return value;
}

function pickLoginBuffer(data: unknown): string {
...
function attachRotatedCredentials(
  error: unknown,
  refreshToken: string,
  accessToken: string,
): RotatedCredentialError {
  const credentialError = (error instanceof Error ? error : new Error(String(error))) as RotatedCredentialError;
...
class WxLoginService {
  async createQrSession(): Promise<{ session: WxLoginSession; qr: Buffer }> {
    const cookies: CookieJar = /* @__PURE__ */ new Map();
  ...
  async poll(session: WxLoginSession): Promise<WxPollStatus> {
  ...
  async confirm(session: WxLoginSession): Promise<{ openid: string; loginBuffer: string }> {
    ...
    const nonce = String(crypto.randomInt(1e3, 1e4));
    ...
    const data = asRecord(JSON.parse(response.body.toString("utf8")) as unknown);
  ...
  async fetchUserInfo(session: WxLoginSession): Promise<DataRecord> {
    ...
    const response = await request(USER_INFO_URL, session.cookies, {
      headers: {
        ...
        "Ual-Access-Requestid": String(crypto.randomInt(1e3, 1e4)),
    ...
    return asRecord(JSON.parse(response.body.toString("utf8")) as unknown);
  }
  async issueCode(session: Pick<WxLoginSession, "loginBuffer">, appId: string): Promise<string> {
  ...
  async refreshLoginBuffer(session: WxLoginSession): Promise<{
    loginBuffer: string;
    refreshtoken: string;
    accesstoken: string;
  }> {
    ...
    const data = asRecord(JSON.parse(response.body.toString("utf8")) as unknown);
    if (data.code !== 0) throw new Error(`WeChat token refresh failed: code=${String(data.code)} msg=${String(data.msg)}`);
    ...
    const nonce2 = String(crypto.randomInt(1e3, 1e4));
    const sig2 = crypto.createHash("md5").update(`${lbPayload}${ts2}${LOGIN_BUFFER_ACCESS_KEY}${nonce2}`).digest("hex");
    try {
      ...
      const lbData = asRecord(JSON.parse(lbResp.body.toString("utf8")) as unknown);
