...
import type {BinaryLike, CipherGCM, DecipherGCM, ECDH} from "node:crypto";
...
interface ProtocolSession extends ManualSession {
  deviceId: Buffer;
  hostAppId: Buffer;
  psk: Buffer;
  ticket: Buffer;
...
const vi = (n: bigint | number): Buffer => {
  let v = BigInt(n);
  const out: number[] = [];
  do {
    let b = Number(v & 127n);
...
const pbl = (f: number, b: Uint8Array): Buffer => Buffer.concat([vi(f * 8 + 2), vi(b.length), b]);
const pbv = (f: number, n: bigint | number): Buffer => Buffer.concat([vi(f * 8), vi(n)]);
function rvi(b: Buffer, o: number): [bigint, number] {
  let n = 0n;
  let s = 0n;
  for (; o < b.length; o++) {
    const x = b[o];
    n |= BigInt(x & 127) << s;
    if (!(x & 128)) return [n, o + 1];
    s += 7n;
  }
  throw new Error("truncated varint");
}
function pbf(b: Buffer): Map<number, ProtobufValue> {
  const out = new Map<number, ProtobufValue>();
  let o = 0;
  while (o < b.length) {
    const [tag, a] = rvi(b, o);
    o = a;
    ...
    if ((tag & 7n) === 0n) {
      ...
      o = z;
      const e = o + Number(n);
      if (e > b.length) throw new Error("truncated protobuf");
...
const hmac = (key: BinaryLike, data: BinaryLike): Buffer => crypto.createHmac("sha256", key).update(data).digest();
function expand(secret: BinaryLike, label: string, context: Uint8Array, size: number): Buffer {
  const out: Buffer[] = [];
...
function extract(salt: BinaryLike, data: BinaryLike): Buffer {
  return hmac(salt, data);
}
function nonce(iv: Uint8Array, seq: bigint | number): Buffer {
  const n = Buffer.from(iv);
  const s = Buffer.alloc(8);
  s.writeBigUInt64BE(BigInt(seq));
  for (let i = 0; i < 8; i++) n[n.length - 8 + i] ^= s[i];
  return n;
}
function gcm(key: Buffer, iv: Buffer, seq: bigint | number, type: number, data: Buffer, decrypt = false): Buffer {
  ...
  if (decrypt) {
    const d = crypto.createDecipheriv(algo, key, nonce(iv, seq)) as DecipherGCM;
    d.setAAD(aad);
    d.setAuthTag(data.subarray(-16));
    return Buffer.concat([d.update(data.subarray(0, -16)), d.final()]);
  }
  const c = crypto.createCipheriv(algo, key, nonce(iv, seq)) as CipherGCM;
  c.setAAD(aad);
  return Buffer.concat([c.update(data), c.final(), c.getAuthTag()]);
}
function layout(key: Buffer, plain: Buffer, aad: Buffer = Buffer.alloc(0)): Buffer {
  const iv = crypto.randomBytes(12);
  const c = crypto.createCipheriv(`aes-${key.length * 8}-gcm`, key, iv) as CipherGCM;
  c.setAAD(aad);
  return Buffer.concat([c.update(plain), c.final(), iv, c.getAuthTag()]);
}
function unlayout(key: Buffer, blob: Buffer, aad: Buffer = Buffer.alloc(0)): Buffer {
  const split = blob.length - 28;
  const d = crypto.createDecipheriv(`aes-${key.length * 8}-gcm`, key, blob.subarray(split, split + 12)) as DecipherGCM;
  d.setAAD(aad);
  d.setAuthTag(blob.subarray(split + 12));
  return Buffer.concat([d.update(blob.subarray(0, split)), d.final()]);
}
const rec = (type: number, body: Buffer): Buffer => {
...
function records(data: Buffer): ProtocolRecord[] {
  const out: ProtocolRecord[] = [];
  for (let o = 0; o + 5 <= data.length; ) {
    const n = data.readUInt16BE(o + 3);
    if (data.readUInt16BE(o + 1) !== REC || o + 5 + n > data.length) break;
    out.push({ type: data[o], body: data.subarray(o + 5, o + 5 + n) });
    o += 5 + n;
  }
  return out;
}
const hs = (type: number, body: Buffer): Buffer => {
  const h = Buffer.alloc(5);
  h.writeUInt32BE(body.length + 1);
  h[4] = type;
  return Buffer.concat([h, body]);
};
function splitHs(b: Buffer): ProtocolRecord {
  if (b.length < 5 || b.readUInt32BE() + 4 > b.length) throw new Error("invalid handshake");
  return { type: b[4], body: b.subarray(5) };
}
function lz4Literal(data: Buffer): Buffer {
  if (data.length < 15) return Buffer.concat([Buffer.from([data.length << 4]), data]);
  const xs: number[] = [240];
  for (let n = data.length - 15; n >= 255; n -= 255) xs.push(255);
  xs.push((data.length - 15) % 255);
  return Buffer.concat([Buffer.from(xs), data]);
}
function lz4(data: Buffer): Buffer {
  const out: number[] = [];
  for (let i = 0; i < data.length; ) {
    ...
    for (let e = i + n; i < e; i++) out.push(data[i]);
    ...
    for (let j = 0; j < m; j++) out.push(out[out.length - off]);
  }
  return Buffer.from(out);
}
function wpkg(ints: Record<number, bigint | number>, bytes: Record<number, Buffer>): Buffer {
  const a: Buffer[] = [vi(1)];
  for (const k of Object.keys(ints).map(Number).sort((x, y) => x - y)) a.push(vi(k), vi(ints[k]));
  a.push(vi(0));
  for (const k of Object.keys(bytes).map(Number).sort((x, y) => x - y)) a.push(vi(k), vi(bytes[k].length), bytes[k]);
  const p = Buffer.concat(a);
  return Buffer.concat([p, vi(0), vi(p.length + 1)]);
}
function readWpkg(b: Buffer): number {
  let o = 0;
  [, o] = rvi(b, o);
  for (; ; ) {
    const [f, n] = rvi(b, o);
...
function short(cmd: number, seq: number, body: Buffer): Buffer {
...
function parseShort(b: Buffer): { cmd: number; body: Buffer } {
  if (b.length < 16 || b.readUInt32BE() > b.length) throw new Error("invalid shortlink");
  return { cmd: b.readUInt32BE(8), body: b.subarray(16, b.readUInt32BE()) };
}
function ecdh(): { e: ECDH; pub: Buffer } {
  const e = crypto.createECDH("prime256v1");
  return { e, pub: e.generateKeys() };
}
function ch(pub1: Buffer, pub2: Buffer): Buffer {
...
function pskClientHello(ticket: Buffer, timestamp: number): Buffer {
...
async function socket(host: string, port: number, timeout = 3e4): Promise<{
  s: net.Socket;
  take: () => Promise<ProtocolRecord>;
...
function keys(secret: BinaryLike, label: string, hash: Uint8Array, size = 56): {
  ck: Buffer;
  sk: Buffer;
  ci: Buffer;
  si: Buffer;
...
function oneWayKeys(secret: BinaryLike, label: string, hash: Uint8Array): { key: Buffer; iv: Buffer } {
  const z = expand(secret, label, hash, 28);
  return { key: z.subarray(0, 16), iv: z.subarray(16, 28) };
}
function manualRequest(buffer: string, app: Buffer): { req: Buffer; device: Buffer; host: Buffer } {
  const raw = Buffer.from(buffer, "base64");
  const f = pbf(raw);
  const ticket = f.get(1);
  const device = f.get(2);
  const host = f.get(3);
  if (!Buffer.isBuffer(ticket) || !ticket.length || !Buffer.isBuffer(device) || !device.length) throw new Error("invalid login buffer");
...
function hybrid(plain: Buffer): { temp: HybridState; wire: Buffer } {
  const x = ecdh();
  const secret = crypto.createHash("sha256").update(x.e.computeSecret(SERVER_PUB)).digest();
  const h1 = crypto.createHash("sha256").update(Buffer.concat([U8("1"), U8("415"), x.pub])).digest();
  ...
  const h2 = crypto.createHash("sha256").update(Buffer.concat([U8("1"), U8("415"), x.pub, encKey])).digest();
...
function requiredField(fields: Map<number, ProtobufValue>, field: number, name: string): Buffer {
  const value = fields.get(field);
  if (!Buffer.isBuffer(value)) throw new Error(`${name} is missing (fields: ${[...fields.keys()].join(",") || "none"})`);
  return value;
}
function parseManual(body: Buffer, temp: HybridState): ManualSession {
  let hybridResponse: Buffer | undefined;
  try {
    const offset = readWpkg(body);
    if (offset < body.length && body[offset] === 10) hybridResponse = body.subarray(offset);
  } catch {
  }
  if (!hybridResponse) {
  ...
  if (typeof uin !== "bigint") throw new Error("ManualAuthResponse uin is missing");
  return {
...
function jsPlain(uin: bigint, appId: string, host: Buffer): Buffer {
  const mac = crypto.randomBytes(6);
  mac[0] = (mac[0] | 2) & 254;
  const dev = U8((mac.toString("hex").match(/../g) || []).join("-").toUpperCase());
  const uin32 = BigInt.asUintN(32, uin);
  const info = (name: string): Buffer => Buffer.concat([pbl(1, U8("sessionkey")), pbv(2, uin32), pbl(3, dev), pbv(4, 1661404927), pbl(5, U8(name)), pbv(6, 0)]);
...
function envelope(s: ProtocolSession, plain: Buffer): Buffer {
...
async function targets(kind: "long" | "short"): Promise<ProtocolTarget[]> {
  ...
  const ports = Array.isArray(portList) ? portList.filter((port): port is number => typeof port === "number") : [];
...
async function getNativeWxLoginCode(loginBuffer: string, appId: string): Promise<string> {
  ...
  for (const t of (await targets("long")).slice(0, 12)) {
    try {
      ...
      if (ext.length < 78 || ext[0] !== 1) throw new Error("invalid ServerHello key-share extension");
      ...
      let certHash: Buffer | undefined;
      const ticketEntries: Buffer[] = [];
      let rxSeq = 1;
      for (; ; ) {
        const r = await c.take();
        const plain = gcm(hsKeys.sk, hsKeys.si, rxSeq++, r.type, r.body, true);
        const x = splitHs(plain);
        if (x.type !== 20) transcript.push(plain);
        if (x.type === 15) certHash = crypto.createHash("sha256").update(Buffer.concat(transcript)).digest();
        if (x.type === 4) {
          let o = 1;
        ...
        if (x.type === 20) {
          ...
          if (!s.sendKey.length || !s.recvKey.length || !s.uin || !firstTicket) throw new Error("ManualAuth did not return a usable session");
  ...
  const early = async (target: ProtocolTarget): Promise<string> => {
    const ts = Math.floor(Date.now() / 1e3);
    const ticket = activeSession.ticket;
    const hello = pskClientHello(ticket, ts);
    const ek = oneWayKeys(activeSession.psk, "early data key expansion", crypto.createHash("sha256").update(hello).digest());
...
`;
    ...
    const chunks: Buffer[] = [];
    c.s.on("data", (chunk: Buffer) => chunks.push(Buffer.from(chunk)));
    ...
    for (const transcript of transcripts) {
      const hk = oneWayKeys(activeSession.psk, "handshake key expansion", crypto.createHash("sha256").update(transcript).digest());
  ...
  for (const t of await targets("short")) try {
