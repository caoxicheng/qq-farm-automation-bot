...
const native_protocol_exports = {};
__export(native_protocol_exports, {
  getNativeWxLoginCode: () => getNativeWxLoginCode
});
module.exports = __toCommonJS(native_protocol_exports);
const import_node_crypto = __toESM(require("node:crypto"));
const import_node_net = __toESM(require("node:net"));
...
const hmac = (key, data) => import_node_crypto.default.createHmac("sha256", key).update(data).digest();
...
function gcm(key, iv, seq, type, data, decrypt = false) {
  ...
  if (decrypt) {
    const d = import_node_crypto.default.createDecipheriv(algo, key, nonce(iv, seq));
    d.setAAD(aad);
    d.setAuthTag(data.subarray(-16));
    return Buffer.concat([d.update(data.subarray(0, -16)), d.final()]);
  }
  const c = import_node_crypto.default.createCipheriv(algo, key, nonce(iv, seq));
  c.setAAD(aad);
...
function layout(key, plain, aad = Buffer.alloc(0)) {
  ...
  const c = import_node_crypto.default.createCipheriv(`aes-${key.length * 8}-gcm`, key, iv);
  c.setAAD(aad);
...
function unlayout(key, blob, aad = Buffer.alloc(0)) {
  ...
  const d = import_node_crypto.default.createDecipheriv(`aes-${key.length * 8}-gcm`, key, blob.subarray(split, split + 12));
  d.setAAD(aad);
...
function ecdh() {
  const e = import_node_crypto.default.createECDH("prime256v1");
  return { e, pub: e.generateKeys() };
}
function ch(pub1, pub2) {
  const r = import_node_crypto.default.randomBytes(32);
...
function pskClientHello(ticket, timestamp) {
  ...
  const body = Buffer.concat([
    ...
    import_node_crypto.default.randomBytes(32),
    u32(timestamp),
...
async function socket(host, port, timeout = 3e4) {
  const s = import_node_net.default.createConnection({ host, port });
  const chunks = [];
  let ended = false;
  s.on("data", (x) => chunks.push(Buffer.from(x)));
  s.on("end", () => {
    ended = true;
  });
  await new Promise((ok, no) => {
    s.once("connect", ok);
    s.once("error", no);
    setTimeout(() => no(new Error("connection timeout")), timeout);
  });
  let offset = 0;
  async function take() {
...
function hybrid(plain) {
  const x = ecdh();
  const secret = import_node_crypto.default.createHash("sha256").update(x.e.computeSecret(SERVER_PUB)).digest();
  const h1 = import_node_crypto.default.createHash("sha256").update(Buffer.concat([U8("1"), U8("415"), x.pub])).digest();
  const cek = import_node_crypto.default.randomBytes(32);
  ...
  const h2 = import_node_crypto.default.createHash("sha256").update(Buffer.concat([U8("1"), U8("415"), x.pub, encKey])).digest();
...
function parseManual(body, temp) {
  ...
  const secret = import_node_crypto.default.createHash("sha256").update(temp.e.computeSecret(peer)).digest();
  const aad = import_node_crypto.default.createHash("sha256").update(Buffer.concat([temp.okm.subarray(24), temp.comp, U8("415"), peer, U8(String(cred))])).digest();
...
function jsPlain(uin, appId, host) {
  const mac = import_node_crypto.default.randomBytes(6);
  mac[0] = (mac[0] | 2) & 254;
...
async function targets(kind) {
  ...
  const item = d?.dns?.domainlist?.find((x) => x.name === (kind === "long" ? "longcloud.weixin.com" : "shortcloud.weixin.com"));
  const proto = kind === "long" ? "mmtlsovertcp" : "http";
  const ports = item?.protocollist?.find((x) => x.name === proto)?.portlist || [];
  ...
  const ips = (item?.iplist || []).map((x) => x.ip);
...
async function getNativeWxLoginCode(loginBuffer, appId) {
  const { req, device, host } = manualRequest(loginBuffer, import_node_crypto.default.randomBytes(32));
  let session;
  const failures = [];
  for (const t of (await targets("long")).slice(0, 12)) {
    try {
      ...
      const secret = import_node_crypto.default.createHash("sha256").update(a.e.computeSecret(ext.subarray(13, 78))).digest();
      const transcript = [hello, sh.body];
      const hsKeys = keys(secret, "handshake key expansion", import_node_crypto.default.createHash("sha256").update(Buffer.concat(transcript)).digest());
      let certHash;
      const ticketEntries = [];
      let rxSeq = 1;
      for (; ; ) {
        ...
        if (x.type === 15) certHash = import_node_crypto.default.createHash("sha256").update(Buffer.concat(transcript)).digest();
        if (x.type === 4) {
          let o = 1;
        ...
        if (x.type === 20) {
          const hash = import_node_crypto.default.createHash("sha256").update(Buffer.concat(transcript)).digest();
  ...
  const early = async (target) => {
    ...
    const ek = oneWayKeys(session.psk, "early data key expansion", import_node_crypto.default.createHash("sha256").update(hello).digest());
...
`;
    ...
    for (const transcript of transcripts) {
      const hk = oneWayKeys(session.psk, "handshake key expansion", import_node_crypto.default.createHash("sha256").update(transcript).digest());
