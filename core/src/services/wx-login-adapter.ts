...
import type { AccountsData, StoredAccount } from '../models/account-repository';
import { createModuleLogger } from './logger';
import {
    
    WxLoginService
    
} from './wx-login/service';
import type {RotatedCredentialError, WxLoginSession} from './wx-login/service';
...
const { getAccounts, addOrUpdateAccount } = require('../models/store') as {
    getAccounts: () => AccountsData | StoredAccount[];
    addOrUpdateAccount: (account: unknown) => AccountsData;
};

interface WxSessionEntry {
    owner: string;
    ...
    nickname?: string;
    confirmed?: boolean;
    confirmPromise?: Promise<void> | null;
}

interface OwnedWxSession {
    uuid: string;
    entry: WxSessionEntry;
}

export interface PendingWxInfo {
    sessionId: string;
    loginBuffer: string;
    refreshtoken: string;
    accesstoken: string;
...
export interface FarmCodeOptions {
    sessionId?: unknown;
    owner?: unknown;
    accountId?: unknown;
}
...
type AdapterResult<T> = AdapterSuccess<T> | AdapterFailure;
type KeepAliveResult = EmptyAdapterSuccess | AdapterFailure;
...
const wxSessions = new Map<string, WxSessionEntry>();
const farmCodeRequests = new Map<string, Promise<AdapterResult<FarmCodeData>>>();
...
function asRotatedCredentialError(error: unknown): RotatedCredentialError {
    return (error instanceof Error ? error : new Error(String(error))) as RotatedCredentialError;
}

function findAccountByWxid(openid: unknown, accountId: unknown = ''): StoredAccount | null {
...
async function withAccountCredentialLock<T>(
    openid: unknown,
    accountId: unknown,
    operation: () => Promise<T>,
): Promise<T> {
...
function findOwnedWxSession(
    sessionId: unknown,
    openid: unknown,
    owner: unknown,
    requireLoginBuffer = false,
): OwnedWxSession | null {
...
function buildPendingWxInfo(matched: OwnedWxSession | null): PendingWxInfo | null {
...
function pickUserInfoValue(info: unknown, keys: readonly string[]): string {
 ...
 * 返回会话快照，不在账号持久化成功前消费。
 */
function peekPendingWxInfo(sessionId: unknown, openid: unknown, owner: unknown): PendingWxInfo | null {
    return buildPendingWxInfo(findOwnedWxSession(sessionId, openid, owner, true));
}

function consumePendingWxInfo(sessionId: unknown, openid: unknown, owner: unknown): boolean {
...
function humanizeWxCodeError(raw: unknown): string {
 ...
 */
async function getQRCode(owner: unknown) {
 ...
 */
async function checkQR(uuid: unknown, owner: unknown) {
 ...
 */
async function issueFarmCode(openid: unknown, options: FarmCodeOptions = {}): Promise<AdapterResult<FarmCodeData>> {
    if (!openid) {
...
async function getFarmCode(openid: unknown, options: FarmCodeOptions = {}): Promise<AdapterResult<FarmCodeData>> {
    const targetOpenid = String(openid || '');
 ...
 */
async function getAccountAvatar(openid: unknown): Promise<Response | null> {
    if (!openid) return null;
 ...
 */
async function keepWxCredentialAlive(acc: unknown): Promise<KeepAliveResult> {
