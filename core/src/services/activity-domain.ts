...
const import_constellation_2026072701 = {
  default: JSON.parse(fs.readFileSync(getResourcePath('activity-data', 'constellation-2026072701.json'), 'utf8'))
};
...
const lastConstellationState = /* @__PURE__ */ new Map<string, DynamicRecord>();
const lastConstellationDynamicState = /* @__PURE__ */ new Map<string, DynamicRecord>();
function compareInt64(left: unknown, right: unknown): number {
...
function textContent(value: unknown): { title: string; paragraphs: string[] } {
  ...
  try {
    ...
    const paragraphs = rawParagraphs.filter((entry: unknown) => typeof entry === "string").map(plainText).filter(Boolean);
...
function parseActivityExtra(value: unknown): unknown {
  const parsed = parseJsonText(value);
  return parseNestedJsonValue(parsed);
}
function activityDto(activity: unknown): DynamicRecord {
  return mapActivityDto(activity, parseActivityExtra);
}
function passDto(pass: DynamicRecord | null | undefined): DynamicRecord | null {
  if (!pass) return null;
  ...
  const nodes = (Array.isArray(pass.nodes) ? pass.nodes : []).map((node: DynamicRecord) => {
...
function rawConstellationNode(node: DynamicRecord | null | undefined): DynamicRecord {
  return {
...
function rawConstellationGroup(group: DynamicRecord | null | undefined): DynamicRecord {
  return {
...
function constellationStateIdentity(seasonReply: DynamicRecord, activity: DynamicRecord): DynamicRecord {
  return {
...
function loadMergedConstellationState(seasonReply: DynamicRecord, activity: DynamicRecord): DynamicRecord {
...
function constellationDto(
  activity: DynamicRecord,
  serverTimeValue: unknown,
  data: DynamicRecord | null | undefined,
  confirmedState: DynamicRecord | null | undefined
): DynamicRecord {
  ...
  const dynamicNodes = new Map<string, DynamicRecord>(nodes.map((node: DynamicRecord) => [int64String(node?.node_id), node]));
  const dynamicGroups = new Map<string, DynamicRecord>((Array.isArray(data?.groups) ? data.groups : []).map((group: DynamicRecord) => [int64String(group?.group_id), group]));
  ...
  const groups = import_constellation_2026072701.default.groups.map((group: DynamicRecord) => {
...
async function querySeason(timeoutOrOptions: unknown = 20000): Promise<DynamicRecord> {
...
async function querySolarTerms(timeoutOrOptions: unknown = 20000): Promise<DynamicRecord> {
...
function findSeasonActivity(seasonReply: DynamicRecord | null | undefined, typeCode: string): DynamicRecord | null {
  const activities = Array.isArray(seasonReply?.season_info?.activities) ? seasonReply.season_info.activities : [];
  return activities.find((activity: DynamicRecord) => int64String(activity?.type) === typeCode) || null;
}
function normalizeSeason(reply: DynamicRecord): DynamicRecord {
...
function normalizeSolarTerms(reply: DynamicRecord): DynamicRecord {
  const serverTime = int64Number(reply?.server_time);
  const terms = (Array.isArray(reply?.terms) ? reply.terms : [])
    .map(solarTermDto)
    .filter((term: DynamicRecord | null): term is DynamicRecord => term !== null);
  ...
  return {
    ...
    configs: configs.map((config: DynamicRecord) => ({
...
function readBagBalances(bagReply: unknown, currencyIds: string[]): BalanceMap {
  const requestedIds = new Set<string>(currencyIds);
  const balances = new Map<string, bigint>(currencyIds.map((id: string) => [id, 0n]));
...
function isExplicitlyUnavailableShopStatus(_statusCode: unknown): boolean {
  return false;
}
function normalizeShopFromReply(
  seasonReply: DynamicRecord,
  shopActivity: DynamicRecord,
  reply: DynamicRecord,
  balances: BalanceMap | null
): DynamicRecord {
  const goods = Array.isArray(reply.data?.catalog?.goods) ? reply.data.catalog.goods : [];
  const currencyIds = Array.from(new Set<string>(goods.map((entry: DynamicRecord) => int64String(entry?.cost?.item_id)).filter((id: string) => id !== "0")));
  const balanceKnown = balances !== null;
  const activityId = int64String(reply.activity_id);
  const goodsDtos = goods.map((entry: DynamicRecord) => {
  ...
  const exchangeableCount = goodsDtos.filter((entry: DynamicRecord) => entry.exchangeable).length;
  const affordableCount = goodsDtos.filter((entry: DynamicRecord) => entry.exchangeable && (!entry.maxExchangeCountKnown || BigInt(entry.maxExchangeCount) > 0n)).length;
  return {
    activityId,
    ...
    categories: Array.from(new Set<string>(goods.map((entry: DynamicRecord) => bytesToText(entry.category)).filter(Boolean))),
    goods: goodsDtos,
    action: {
      supported: true,
...
async function queryShopCatalog(shopActivity: DynamicRecord, timeoutOrOptions: unknown = 20000): Promise<DynamicRecord> {
...
async function queryShopFromSeason(
  seasonReply: DynamicRecord,
  bagReply: unknown = null,
  timeoutOrOptions: unknown = 20000
...
function settledError(entry: PromiseSettledResult<unknown>): string | null {
...
function buildActions(
  ...
  const lightableGroups = groups.filter((group: DynamicRecord) => group.visualState === "lightable");
  const attemptableGroups = groups.filter((group: DynamicRecord) => group.visualState === "lightable" || group.visualState === "claimableUnknown");
  const currentGroups = groups.filter((group: DynamicRecord) => group.order === constellation?.currentDay);
  const availabilityKnown = lightableGroups.length > 0 || currentGroups.length > 0 && currentGroups.every((group: DynamicRecord) => group.stateKnown);
  const hasClaimableSolar = !!solarTerms?.terms?.some((term: DynamicRecord) => term.canClaim);
  return {
    claimPass: {
      supported: true,
      enabled: hasPass,
...
async function buildActivityCenterSnapshot(shopOverride: DynamicRecord | null = null): Promise<DynamicRecord> {
  ...
  const constellationIdentity = rawSeason && constellationActivity
    ...
    : null;
  const constellation = rawSeason && constellationActivity && constellationIdentity ? constellationDto(
...
const snapshotCoordinator = createActivitySnapshotCoordinator<DynamicRecord, DynamicRecord>(buildActivityCenterSnapshot);
function getActivityCenterSnapshot(shopOverride: DynamicRecord | null = null): Promise<DynamicRecord> {
  return snapshotCoordinator.getSnapshot(shopOverride);
}
async function getFreshActivityCenterSnapshot(shopOverride: DynamicRecord | null = null): Promise<DynamicRecord> {
  return snapshotCoordinator.getFreshSnapshot(shopOverride);
}
async function captureFreshQingMeiMutationSnapshot(): Promise<DynamicRecord> {
...
async function getCurrentStarSandShop(): Promise<DynamicRecord> {
  return queryShopFromSeason(await querySeason());
}
async function getCurrentSolarTerms(): Promise<DynamicRecord> {
...
async function claimBattlePassRewards(): Promise<DynamicRecord> {
...
async function exchangeStarSandGoods(goodsIdInput: unknown, countInput: unknown): Promise<DynamicRecord> {
  ...
  return serializeMutation(async () => {
    ...
    const rawGoods = catalogGoods.find((entry: DynamicRecord) => int64String(entry?.goods_id) === goodsId);
    ...
    const normalizedGoods = shopBefore.goods.find((entry: DynamicRecord) => entry.id === goodsId);
    ...
    const responseCurrencyIds = Array.from(new Set<string>(reply.data.catalog.goods.map((entry: DynamicRecord) => int64String(entry?.cost?.item_id)).filter((id: string) => id !== "0")));
...
async function lightConstellation(): Promise<DynamicRecord> {
  return serializeMutation(async () => {
    const seasonReply = await querySeason(ACTIVITY_READ_TIMEOUT_MS);
    const activity = findSeasonActivity(seasonReply, CONSTELLATION_ACTIVITY_TYPE);
    ...
    try {
      ...
      const gatewayError = error as { code?: number };
      if (!(error instanceof GatewayError) || gatewayError.code !== 1034038 || !activityActive || currentDay < 1 || currentDay > 28) {
        throw error;
      }
      ...
      try {
        ...
        persistenceWarning2 = errorMessage(persistenceError) || "\u89C2\u661F\u72B6\u6001\u6301\u4E45\u5316\u5931\u8D25";
      }
    ...
    try {
      ...
      persistenceWarning = errorMessage(persistenceError) || "\u89C2\u661F\u72B6\u6001\u6301\u4E45\u5316\u5931\u8D25";
    }
    const snapshot = await getFreshActivityCenterSnapshot();
    return {
      outcome: "lighted",
...
async function claimSolarTerm(termId: string): Promise<DynamicRecord> {
  return serializeMutation(async () => {
    if (!/^[1-9]\d*$/.test(termId)) throw new Error("termId \u5FC5\u987B\u662F\u6B63\u5341\u8FDB\u5236\u6574\u6570");
    const solarReply = await querySolarTerms(ACTIVITY_READ_TIMEOUT_MS);
    const term = (Array.isArray(solarReply?.terms) ? solarReply.terms : []).find((entry: DynamicRecord) => int64String(entry?.term_id) === termId);
