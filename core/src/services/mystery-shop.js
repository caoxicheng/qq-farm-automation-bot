...
async function readBagBalances(currencyIds) {
    const requested = new Set(currencyIds.map((id) => String(id)));
    const balances = new Map(currencyIds.map((id) => [String(id), 0]));
    try {
        const bagReply = await getBag();
        for (const item of getBagItems(bagReply)) {
            const id = int64String(item && (item.id ?? item.item_id));
            if (!requested.has(id)) continue;
            const count = BigInt(int64String(item && (item.count ?? item.item_count)) || '0');
...
async function buyNpcGoods(npcId, count = 1) {
...
async function getMysteryShopSnapshot() {
...
// 购买：返回购买结果 + 最新快照
async function buyMysteryGoods(npcId, count = 1) {
    const reply = await buyNpcGoods(npcId, count);
    const rewards = Array.isArray(reply.rewards) ? reply.rewards.map((item) => itemDto(item)) : [];
