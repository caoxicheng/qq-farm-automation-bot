...
import { getLevelExpProgress, getLevelExpTable } from '../config/gameConfig';
...
type RecordGoldExpHook = (gold: number, exp: number) => void;

// 统计钩子（可选，admin 未加载时为空）
let recordGoldExpHook: RecordGoldExpHook | null = null;
function setRecordGoldExpHook(hook: RecordGoldExpHook | null): void { recordGoldExpHook = hook; }
...
function updateStatus(data: Partial<StatusData>): void {
    const changed = (Object.keys(data) as Array<keyof StatusData>)
        .some(key => statusData[key] !== data[key]);
    if (changed) Object.assign(statusData, data);
    if (changed) {
 ...
 * 从登录数据更新状态
 */
function updateStatusFromLogin(basic: Partial<StatusData>): void {
    updateStatus({
...
function updateStatusLevel(level: number, exp?: number): void {
    const data: Partial<StatusData> = { level };
