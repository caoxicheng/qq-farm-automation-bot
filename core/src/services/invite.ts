...
import { getShareFilePath } from '../config/runtime-paths';
import { sendMsgAsync } from '../utils/network';
import { types } from '../utils/proto';
import { log, logWarn, sleep, toLong } from '../utils/utils';
import { readTextFile, writeTextFileAtomic } from './json-db';
...
interface ShareInvite extends ParsedShareLink {
    uid: string;
    openid: string;
}
 ...
 * 读取 share.txt 文件并去重
 */
function readShareFile(): ShareInvite[] {
    ...
    try {
        ...
        const seenUids = new Set<string>();  // 用于去重
        ...
        for (const line of lines) {
            ...
            if (parsed.openid && parsed.uid) {
                ...
                if (!seenUids.has(parsed.uid)) {
                    ...
                    invites.push({ ...parsed, uid: parsed.uid, openid: parsed.openid });
                }
            }
        }
        ...
        logWarn('邀请', `读取 share.txt 失败: ${errorMessage(error)}`);
        return [];
    }
}
 ...
 */
async function sendReportArkClick(sharerId: string, sharerOpenId: string, shareSource: string | null) {
...
async function processInviteCodes(): Promise<void> {
    ...
    for (let i = 0; i < invites.length; i++) {
        ...
        try {
            ...
            failCount++;
            logWarn('邀请', `[${i + 1}/${invites.length}] 向 uid=${invite.uid} 发送申请失败: ${errorMessage(error)}`);
        }
