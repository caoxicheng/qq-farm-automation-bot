...
const { asRecord, errorMessage } = require('./service-boundaries');

type FertilizerType = 'organic' | 'normal';
type MallGoodsRecord = Record<string, unknown>;

export interface FertilizerPurchaseOptions {
    buyOrganic?: boolean;
    buyNormal?: boolean;
    organicCount?: number;
...
export interface FertilizerThresholdResult {
    bought: number;
    currentHours?: number;
...
async function getMallListBySlotType(slotType = 1): Promise<MallGoodsRecord> {
    ...
    return asRecord(types.GetMallListBySlotTypeResponse.decode(replyBody));
}

async function purchaseMallGoods(goodsId: unknown, count = 1): Promise<MallGoodsRecord> {
    ...
    return asRecord(types.PurchaseResponse.decode(replyBody));
}

async function getMallGoodsList(slotType = 1): Promise<MallGoodsRecord[]> {
    const mall = await getMallListBySlotType(slotType);
    const raw = Array.isArray(mall.goods_list) ? mall.goods_list : [];
    const goods: MallGoodsRecord[] = [];
    for (const b of raw) {
        try {
            goods.push(asRecord(types.MallGoods.decode(b)));
        } catch {
            // ignore
        }
    }
 ...
 */
function parseMallPriceValue(priceField: unknown): number {
    if (priceField == null) return 0;
    if (typeof priceField === 'number') return Math.max(0, Math.floor(priceField));
    const bytes = Buffer.isBuffer(priceField)
        ? priceField
        : priceField instanceof Uint8Array
            ? Buffer.from(priceField)
            : Buffer.alloc(0);
...
function findOrganicFertilizerMallGoods(goodsList: unknown): MallGoodsRecord | null {
    const list = Array.isArray(goodsList) ? goodsList : [];
    return list
        .map((value): MallGoodsRecord => asRecord(value))
        .find((goods) => toNum(goods.goods_id) === ORGANIC_FERTILIZER_MALL_GOODS_ID) || null;
}

function findInorganicFertilizerMallGoods(goodsList: unknown): MallGoodsRecord | null {
    const list = Array.isArray(goodsList) ? goodsList : [];
    return list
        .map((value): MallGoodsRecord => asRecord(value))
        .find((goods) => toNum(goods.goods_id) === INORGANIC_FERTILIZER_MALL_GOODS_ID) || null;
}

function findFertilizerMallGoods(goodsList: unknown, type: FertilizerType = 'organic'): MallGoodsRecord | null {
    if (type === 'normal') {
...
async function autoBuyFertilizerViaMall(type: FertilizerType = 'organic', targetCount = 0): Promise<number> {
...
async function autoBuyOrganicFertilizer(force = false): Promise<number> {
    const now = Date.now();
...
async function autoBuyFertilizer(force = false, type: FertilizerType = 'organic', targetCount = 0): Promise<number> {
    const now = Date.now();
    if (!force && now - lastBuyAt < BUY_COOLDOWN_MS) return 0;
    lastBuyAt = now;
...
function isDoneTodayByKey(key: unknown): boolean {
    return String(key || '') === getDateKey();
}

async function buyFreeGifts(force = false): Promise<number> {
    const now = Date.now();
    ...
    try {
        ...
        log('商城', `领取免费礼包失败: ${errorMessage(e)}`, {
            module: 'task',
            event: FREE_GIFTS_DAILY_KEY,
            result: 'error',
        });
...
async function checkAndBuyFertilizerByThreshold(
    type: FertilizerType,
    count: number,
    thresholdHours: number,
): Promise<FertilizerThresholdResult> {
    ...
    try {
        ...
        log('商城', `检测化肥容器失败: ${message}`, {
            module: 'mall',
            event: 'check_fertilizer',
            result: 'error',
            error: message,
        });
        return { bought: 0, error: message };
    }
}

async function checkAndBuyFertilizerBoth(options: FertilizerPurchaseOptions = {}) {
    const { getBag, getBagItems, getContainerHoursFromBagItems } = require('./warehouse');
...
function getFertilizerBuyDailyState() {
    return {
        key: 'fertilizer_buy',
...
function getFreeGiftDailyState() {
    return {
